...
if __name__=='__main__':
	...
	if False:
		...
		fcn = FCNNetwork()
		fcn.load_weights(os.path.join(BASE_DIR, 'models\\quadout\\segm_t{0}_era{1}.tfkem'.format(TIME, eraLeftOff)))
		fcn.compile(optimizer=optimizer, loss=loss, metrics=['mae', 'mse', 'msle'])
	...
	for era in range(eraStart, eraEnd):
		...
		tf.keras.models.save_model(fcn, os.path.join(BASE_DIR, 'models\\quadout\\{}.tfkem'.format(NAME)))
		fcn.save_weights(os.path.join(BASE_DIR, 'models\\quadout\\{}.tfkem'.format(NAME)))
