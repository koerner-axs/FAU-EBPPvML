...
class FCNNetwork(tf.keras.Model):
	...
	def  __init__(self):
		...
		self.deconv_ll = tf.keras.layers.Convolution2DTranspose(kernel_size=16, strides=8, filters=3)
...
if __name__=='__main__':
	...
	if False:
		fcn = FCNNetwork()
		fcn.compile(optimizer=optimizer, loss=loss, metrics=['mae', 'mse', 'msle'])
		erasToTrain = 8
		...
		fcn = FCNNetwork()
		fcn.load_weights(os.path.join(BASE_DIR, 'models\\segm_t{0}_era{1}.tfkem'.format(TIME, eraLeftOff)))
		fcn.compile(optimizer=optimizer, loss=loss, metrics=['mae', 'mse', 'msle'])

	dataset = load_dataset('datasets\\segmentation\\Labeled Dataset 4', True)
	...
	for era in range(eraStart, eraEnd):
		...
		tf.keras.models.save_model(fcn, os.path.join(BASE_DIR, 'models\\{}.tfkem'.format(NAME)))
		fcn.save_weights(os.path.join(BASE_DIR, 'models\\{}.tfkem'.format(NAME)))

		Dir = os.path.join(BASE_DIR, 'models\\{}\\'.format(NAME))
		try:
			os.mkdir(Dir)
		except:
			pass
